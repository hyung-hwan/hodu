...
func TestJwt(t *testing.T) {
	...
	if err != nil {
		t.Fatalf("keygen failure - %s", err.Error())
	}
	...
	if jc.Abc != "def" {
		t.Fatal("decoding failure of Abc field")
	}
	if jc.Donkey != "kong" {
		t.Fatal("decoding failure of Donkey field")
	}
	if jc.IssuedAt != 111 {
		t.Fatal("decoding failure of Issued field")
	}
}
